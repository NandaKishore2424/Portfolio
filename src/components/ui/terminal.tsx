...
// From Magic UI (Terminal); adds a title bar and derives sequence state instead of setting it in effects.
import {
  Children,
  createContext,
  useContext,
...
export const AnimatedSpan = ({
  ...
  // The sequence only moves forward, so "reached" stays true once it is.
  const hasStarted =
    !!sequence &&
    itemIndex !== null &&
...
export const TypingAnimation = ({
  ...
  const [startedByView, setStartedByView] = useState(false)
  ...
  const reachedInSequence =
    hasSequence &&
    itemIndex !== null &&
    sequenceStarted &&
    sequenceActiveIndex !== null &&
    sequenceActiveIndex >= itemIndex
  const started = hasSequence && itemIndex !== null ? reachedInSequence : startedByView

  useEffect(() => {
    if (hasSequence && itemIndex !== null) return
    if (startOnView && !isInView) return
    const startTimeout = setTimeout(() => setStartedByView(true), delay)
...
export const Terminal = ({
  ...
  const content = (
    ...
    >
      <div className="border-border flex items-center gap-x-4 border-b px-4 py-3">
        ...
        {title ? (
          <div className="text-muted-foreground flex-1 truncate text-center font-mono text-[11px]">
            {title}
          </div>
        ) : null}
        {title ? <div className="w-[46px]" /> : null}
      </div>
      <pre className="p-4">
